...
def switch_window(switch_frame, size_key):
    ...
    if size_key == 1:
        ...
        label_org.config(image=default_image)
...
# function for event handling changing home icon color
def on_enter_6(enter):
    search_online_img.config(image=btn_active_onlineimg)


def on_leave_6(leave):
    search_online_img.config(image=btn_inactive_onlineimg)


def online_image_convert(arg):
    global img_path
    display_pencil_img(arg)
    switch_window(frame_3, 3)
    img_path=img_path_online


def offline_image_convert(path):
    display_pencil_img(path)
    switch_window(frame_3, 3)


def onBtnClickCallback(arg):
    global img_path_online
    img_path_online=arg
    img = image_loader(arg, 510, 500)
    win2_btn_1.pack(side=LEFT)
    display_img(img)
    win2_btn_2.config(command=lambda: online_image_convert(arg))
    win2_btn_2.pack(side=RIGHT, padx=(120, 0), ipadx=10, ipady=5, pady=(0, 10))


# defining function to hide convert button
def hide_convert_btn():
    win2_btn_2.pack_forget()


# defining function to show upload button
def show_upload_btn():
...
##defining func to display choosen image and call sketch converter
def display_pencil_img(img_path):
    global pencil_img_path
    pencil_img_path = sketch_generator.convertImg(img_path)
    image = image_loader(pencil_img_path, 510, 500)
    image.confg = image
...
def get_path():
    ...
    if extension in ["jpg", "png", "jpeg", "jfef"]:
        ...
        win2_btn_2.config(command=lambda: offline_image_convert(img_path))
        win2_btn_2.pack(side=RIGHT,
                        padx=(120, 0),
                        ipadx=10,
                        ipady=5,
                        pady=(0, 10))
...
def save_image():
    ...
    usr_file_name = os.path.basename(
        img_path)  # getting name of image as in user computer
    # removing extension
...
def fetch_time(image_path):
    time_img = int(
        image_path[24:-4])  # getting time from path at which image converted
...
def recent_images():
    ...
    if len(files) >= 1:
        win4_btn_1.configure(image=recent_image_tup[0],
                             command=lambda: windows_viewer(files[0], 1))

        win4_btn_1.pack()
        win4_label_1.grid(row=0, column=0, padx=8, pady=(40, 0))

        win4_btn_1.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[0], "red"))
        win4_btn_1.bind("<Leave>", on_leave_5)

    if len(files) >= 2:
        win4_btn_2.configure(image=recent_image_tup[1],
                             command=lambda: windows_viewer(files[1], 2))

        win4_btn_2.pack()
        win4_label_2.grid(row=0, column=1, padx=8, pady=(40, 30))

        win4_btn_2.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[1], "green"))
        win4_btn_2.bind("<Leave>", on_leave_5)

    if len(files) >= 3:
        win4_btn_3.configure(image=recent_image_tup[2],
                             command=lambda: windows_viewer(files[2], 3))

        win4_btn_3.pack()
        win4_label_3.grid(row=0, column=2, padx=8, pady=(40, 0))

        win4_btn_3.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[2], "blue"))
        win4_btn_3.bind("<Leave>", on_leave_5)

    if len(files) >= 4:
        win4_btn_4.configure(image=recent_image_tup[3],
                             command=lambda: windows_viewer(files[3], 4))

        win4_btn_4.pack()
        win4_label_4.grid(row=1, column=0, padx=8, pady=(0, 0))

        win4_btn_4.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[3], "purple"))
        win4_btn_4.bind("<Leave>", on_leave_5)

    if len(files) >= 5:
        win4_btn_5.configure(image=recent_image_tup[4],
                             command=lambda: windows_viewer(files[4], 5))

        win4_btn_5.pack()
        win4_label_5.grid(row=1, column=1, padx=8, pady=(70, 0))

        win4_btn_5.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[4], "brown"))
        win4_btn_5.bind("<Leave>", on_leave_5)

    if len(files) == 6:
        win4_btn_6.configure(image=recent_image_tup[5],
                             command=lambda: windows_viewer(files[5], 6))

        win4_btn_6.pack()
        win4_label_6.grid(row=1, column=2, padx=8, pady=(0, 0))

        win4_btn_6.bind("<Enter>",
                        lambda time: on_enter_5(time_date_list[5], "black"))
...
if __name__ == "__main__":
    ...
    # defining button to search image online
    btn_inactive_onlineimg = image_loader("image/search_image_outline.png", 45,
                                          45)
    btn_active_onlineimg = image_loader("image/search_image_filled.png", 45,
                                        45)
    search_online_img = Button(
        frame_2,
        image=btn_inactive_onlineimg,
        command=lambda: imgSE.main(onBtnClickCallback),
        bg="#E4E9F7",
        activebackground="#E4E9F7",
        bd=0,
        relief=RAISED,
    )
    search_online_img.place(x=35, y=600)

    ##binding image online btn to an event
    search_online_img.bind("<Enter>", on_enter_6)
    search_online_img.bind("<Leave>", on_leave_6)
    ...
    label_org = Label(win2_orgimg_frame,
                      bd=0,
                      image=default_image,
                      bg="#E4E9F7")
    ...
    win3_btn_inactive_back = image_loader("image/back_button_outlined.png", 50,
                                          55)
    ...
    # assigning btns
    win4_btn_1 = Button(win4_label_1, cursor="hand2", bd=0)
    win4_btn_2 = Button(win4_label_2, cursor="hand2", bd=0)
    win4_btn_3 = Button(win4_label_3, cursor="hand2", bd=0)
    win4_btn_4 = Button(win4_label_4, cursor="hand2", bd=0)
    win4_btn_5 = Button(win4_label_5, cursor="hand2", bd=0)
    win4_btn_6 = Button(win4_label_6, cursor="hand2", bd=0)
